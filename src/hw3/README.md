...
and then `python -m src.main test hw3` and/or `python tryhw3.py`.
...
The high-level algorithm is already done for you! You can find it near the bottom of `src/hw3/panorama_image.py`, it looks approximately like:
...
This homework may use your previous code from HW1 and HW2. You can use `python -m src.main test hw1` and `python -m src.main test hw2` to test your previous code as well. Also `python -m src.main test hw3` is provided as a sample driver program and does some basic tests and runs most parts of your algorithm. If `python -m src.main test hw3` runs without errors and produces some panoramic images as well as good matches then there's a good chance that you are about 75% done.

First we need to find those corners!

## 1. Harris corner detection ##

We'll be implementing Harris corner detection as discussed in class. Implement this part in `harris_image.py`. The basic algorithm is:
...
#### TO DO ####
Fill in `def make_1d_gaussian(sigma)` and `def smooth_image(im, sigma)` to use this decomposed Gaussian smoothing.

## 1.2 Computer cornerness from structure matrix ##

#### TO DO ####
Fill in `def cornerness_response(S)` to calculate cornerness using the formula mentioned.
...
#### TO DO ####
Fill in `def nms_image(im, w)`.

## 1.4 Complete the Harris detector ##

#### TO DO ####
Fill in the missing sections of `def harris_corner_detector(im, sigma, thresh, nms)`. The function should return a list of descriptors for corners in the image. Code for calculating the descriptors is provided.
...
The rest of the homework takes place in `src/hw3/panorama_image.py`.
...
#### TO DO ####
Implement `def l1_distance(a, b, n)` between two vectors of floats. The vectors and how many values they contain are passed in as arguments.
...
#### TO DO ####
Fill in the first `TODO` in `def match_descriptors(a, an, b, bn, mn)`.
...
To do this, sort the matches based on distance so shortest distance is first. A comparator helper `match_compare` is already implemented for you to use, and in Python you can sort with a key based on `distance`. Next, loop through the matches in order and keep track of which elements in `b` we've seen. If we see one for a second (or third, etc.) time, throw it out! To throw it out, just shift the other elements forward in the list and then remember how many one-to-one matches we have at the end. This can be done with a single pass through the data.

#### TO DO ####
Fill in the second `TODO` in `def match_descriptors(a, an, b, bn, mn)`. Once this is done we can show the matches we discover between the images:
...
#### TO DO ####
Fill in `def point_distance(p, q)` to compute L2 distance between two points.
...
#### TO DO ####
Fill in `def model_inliers(H, m, n, thresh)` to loop over the points, project using the homography, and check if the projected point is within some `thresh` distance of the actual matching point in the other image.
...
#### TO DO ####
Fill in `def compute_homography(matches, n)`.
...
#### TO DO ####
Implement the [Fisher-Yates shuffle](https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_modern_algorithm) in `def randomize_matches(m, n)`.
...
#### TO DO ####
Fill in `def RANSAC(m, n, thresh, k, cutoff)`. Pseudocode is provided in the code file.
...
#### TO DO ####
Fill in `def combine_images(a, b, H)`. With all this working you should be able to create some basic panoramas:
...
#### TO DO  ####
Implement `def cylindrical_project(im, f)` to project an image to cylindrical coordinates and then unroll it. Consult the lecture slides for details about how to implement it. With this implementation you should be able to generate some very big panoramas (the `field*.jpg` panorama) using `field_panorama` in `tryhw3.py`. Attach the generated `field_panorama_5.jpg` in your report. You may also try your own images if you want!

## Turn it in ##

Turn in your `harris_image.py`, `panorama_image.py`, and `report.pdf` on canvas under Homework 3. Your report should contain:
