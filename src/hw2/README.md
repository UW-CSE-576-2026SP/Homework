...
Welcome, it's time for homework 2! This one may be a little harder than the last one so remember to start early! In order to make grading easier, please only edit the files we mention to submit. You will submit the `src/hw2/modify_image.py` file on Canvas.

and then run `python -m src.main test hw2`. For python tests use `python tryhw2.py`.
...
#### TO DO ####
Fill in `def nn_interpolate(im: Image, x: float, y: float, c: int) -> float:`. It should perform nearest neighbor interpolation. Remember to use the closest `int`, not just `int(x)`, because that truncates towards zero. NOTE: your rounding of `x` and `y` should account for the fact that `x` and/or `y` may be integers.

### 1.2 Nearest Neighbor Resizing ###

#### TO DO ####
Fill in `def nn_resize(im: Image, w: int, h: int) -> Image:`. It should:
...
#### TO DO ####
Fill in the function `def bilinear_interpolate(im: Image, x: float, y: float, c: int) -> float:` for bilinear interpolation.

### 1.4 Bilinear Resizing ###

#### TO DO ####
Fill in `def bilinear_resize(im: Image, w: int, h: int) -> Image:` to perform resizing using bilinear interpolation. Try it out again in python:
...
For this section, Python manages memory automatically, so you do not need to manually free local variables at the end of each function. We'll start out by filtering the image with a box filter.
...
#### TO DO ####
Fill in `def l1_normalize(im: Image) -> None:`. This should divide each value in image `im` by the sum of all the values in the image.

#### TO DO ####
Fill in `def make_box_filter(w: int) -> Image:`. We will only use square box filters, so just create a square image of width = height = w and number of channels = 1, with all entries equal to 1. Then use `l1_normalize` to normalize your filter.

### 2.2 Write a convolution function ###

#### TO DO ####
Fill in `def convolve_image(im: Image, filt: Image, preserve: int) -> Image:`. The parameter `preserve` takes a value of either 0 or 1. We will use "clamp" padding for the image borders (get_pixel() already handles it). For this function we have a few scenarios. With normal convolutions we do a weighted sum over an area of the image. With multiple channels in the input image there are a few possible cases we want to handle:
...
Also, `filt` should have the same number of channels as `im` or have 1 channel. This MUST be checked with `assert`.
Hint: You can reduce number of lines of code by using a Python conditional expression.
...
#### TO DO ####
Fill in the functions `def make_highpass_filter() -> Image:`, `def make_sharpen_filter() -> Image:`, and `def make_emboss_filter() -> Image:` to return the example kernels we covered in class. You can try them out on some images! Now, answer Questions 2.3.1 and 2.3.2 in the source file (put your answers just right there).
...
#### TO DO ####
Fill in `def add_image(a: Image, b: Image) -> Image:` to add two images a and b and `def sub_image(a: Image, b: Image) -> Image:` to subtract image b from image a, so that we can perform our transformations of `+` and `-` like this:
...
#### TO DO ####
Fill in the functions `def make_gx_filter() -> Image:` and `def make_gy_filter() -> Image:` to make our sobel filters. They are for estimating the gradient in the x and y direction:
...
#### TO DO ####
Fill in the function `def feature_normalize(im: Image) -> None:`.

### 2.5.3 Calculate gradient magnitude and direction ###

#### TO DO ####
Fill in the function `def sobel_image(im: Image) -> List[Image]:`. It should return two images, the gradient magnitude and direction. The strategy can be found [here](https://en.wikipedia.org/wiki/Sobel_operator#Formulation). We can visualize our magnitude using our normalization function:
...
#### TO DO ####
Write a function `def colorize_sobel(im: Image) -> Image:`. Call `sobel_image(im)`, use the magnitude to specify the saturation and value of an image and the angle (direction) to specify the hue. Then use the `hsv_to_rgb()` function we wrote in Homework 1. The result should look similar to this:
...
#### TO DO (extra credit) ####
Fill in the function `def apply_median_filter(im: Image, kernel_size: int) -> Image:`. NOTE: You may need to write appropriate helper functions. Feel free to create a test function in `tryhw2.py` to check the correctness of your implementation. Hint: Use Python sorting (`sorted`/`.sort()`) to compute the median values.
...
Turn in your `modify_image.py` on canvas under Homework 2. If you attempt the extra credits, also submit `median.jpg` and/or `bilateral.jpg`.
