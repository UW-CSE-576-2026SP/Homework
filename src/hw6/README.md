...
Welcome, it's time for optical flow! For the Python version, run:

```
python -m src.test_hw6
```

You can also run your own quick script in Python to visualize flow results.
...
#### TO DO ####
Fill in `def make_integral_image(im)` as described in the wikipedia article. You may not want to use `get_pixel` unless you do bounds checking so you don't run into trouble.
...
#### TO DO ####
Fill in `def box_filter_image(im, s)` so that every pixel in the output is the average of pixels in a given window size `s`.
...
#### TO DO ####
Fill in `def time_structure_matrix(im, prev, s)`.

### 3.2 Calculating velocity from the structure matrix ###

#### TO DO ####
Fill in `def velocity_image(S, stride)` to use the equation to calculate the velocity of each pixel in the x and y direction. For each pixel, fill in `M`, invert it, and use it to calculate the velocity.

Try calculating the optical flow between two dog images in Python:

    from uwimg import load_image, save_image
    from src.hw6.flow_image import optical_flow_images, draw_flow
...
Turn in your `flow_image.py` on canvas under Homework 6.